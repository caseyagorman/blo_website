...
if ($_REQUEST['submit']) {
	...
	if ($error == '') {
		...
		ini_set('include_path', ini_get('include_path') . ':./private/giggity/backend/');
		ob_start();
		file_put_contents("giggity.log", json_encode($_REQUEST), FILE_APPEND);
		...
		if ($debug) {
			...
			if($json['statusCode'] == 1) {  
				print "<div id='response'>Thank you for submitting your request. A band member will contact you soon. We usually discuss new gigs at our Sunday rehearsals. Please review our <a href='rider.php'>gig rider</a> to get some info about how we operate and what our needs are, so we can minimize confusion and maximize awesomeness!</div>";
...
Please review our <a href='rider.php'>gig rider</a> to get some info about how we operate and what our needs are, so we can minimize confusion and maximize awesomeness!
<br/>
Fill out this Gig Request Form in order to provide us with a description of and logistical details for the event or action.  We generally need at least two weeks advance notice to make a collective decision and logistimasize our brassiness. We usually discuss new gigs at our Sunday rehearsals. 
<p></p>
<form class='gig_request' id='gig_request' method='post'>
<div class='question'><label for='title'>Please give a short title for your event.</label><input autocomplete="off"  type="text" class="required" name="title" value='<?php echo $event_title;?>' /></div>
...
<div class='question'><label for='url'>If there is a public website/link about this event, paste it here.</label><input autocomplete="off" type="text" name="url" value='<?php echo $url;?>' /></div>
