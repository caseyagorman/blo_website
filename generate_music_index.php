...
foreach ($list as $song) { 
	...
	if (isset($song['score'])) {
		print "\t\t<li><a class=\"pdf\" target='_blank' href=\"$song[score]\">Score</a></li>\n";
	}
	if (isset($song['source'])) {
		print "\t\t<li><a class=\"lily\" target='_blank' href=\"$song[source]\">Lilypond Source</a></li>\n";
	}
	if (isset($song['mus'])) {
		print "\t\t<li><a class=\"mus\" target='_blank' href=\"$song[mus]\">Finale Source</a></li>\n";
	}
	if (isset($song['midi'])) {
		print "\t\t<li><a class=\"midi\" target='_blank' href=\"$song[midi]\">Midi</a></li>\n";
	}
	if (isset($song['lyrics'])) {
		print "\t\t<li><a class=\"pdf\" target='_blank' href=\"$song[lyrics]\">Lyrics</a></li>\n";
	}
	if (isset($song['zip'])) {
		print "\t\t<li><a class=\"zip\" target='_blank' href=\"$song[zip]\">Zipfile containing all files</a></li>\n";
	}
	if (reset($song['files'])) { 
		print "
		<li>Files
		...
		foreach($song['files'] as $file) { 
			print "\t\t<li><a class=\"pdf\" target='_blank' href=\"$song[dir]/$file\">$file</a></li>\n";
		}
		print "\n\t</ul>\n";
	}
	if (reset($song['parts'])) { 
		...
		foreach($song['parts'] as $part) { 
			...
			foreach($part['keys'] as $key) { 
				...
				foreach(array_keys($key) as $page) {
					...
					print "
						<li class=\"format\"><a class=\"pdf\" target='_blank' href=\"$key[$page]\">$page</a></li>";
				}
				print "
					</ul></li>";
			}
